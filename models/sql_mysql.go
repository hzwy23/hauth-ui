...
func init() {
	...
	if "mysql" == defdb {
		sys_rdbms_001 = `SELECT f.authorization_level FROM sys_user_info t
							inner join sys_org_info i
							on t.org_unit_id = i.org_unit_id
							inner join sys_domain_share_info f
							on f.domain_id = ? and i.domain_id = f.target_domain_id
							where t.user_id = ?`
		...
		//							) limit ?,?`
		//
		//sys_rdbms_057 = `select t.user_id,t.user_name,i.org_unit_desc
		//							from sys_user_info t
		//							inner join sys_org_info i on i.org_unit_id = t.org_unit_id
		//							left join sys_domain_info di on i.domain_id = di.domain_id
		//							where t.user_id <> ? and di.domain_id = ?
		...
		* 获取角色拥有的资源信息
		...
		sys_rdbms_094 = `select r.user_id, t.role_id,t.code_number,t.role_name from sys_role_info t
						...
						where r.user_id = ?`
		sys_rdbms_095 = `select '', t.role_id,t.code_number,t.role_name from sys_role_info t
						where not exists(
                        select 1 from sys_role_user_relation r
